...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(kLotteryDateSuccessedFirstNotifications:) name:kLotteryDateSuccessedFristNotification object:nil];
...
- (void)kLotteryDateSuccessedFirstNotifications:(NSNotification *)notification{
    ...
    if ([value isKindOfClass:[NSArray class]]) {
        ...
        LottoryCategoryModel *model = [arr objectAtIndex:0];
...
- (TopMenuView *)ninaSelectionView {
    ...
    if (!_topMenuView) {
        ...
        _topMenuView.defaultSelected = [LNLotteryCategories sharedInstance].categorySelectedIndex? [LNLotteryCategories sharedInstance].categorySelectedIndex:1;
